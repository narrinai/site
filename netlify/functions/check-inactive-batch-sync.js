// Synchronous batch function to check inactive chats from 3 weeks ago to yesterday
// Processes as many as possible within the 26-second timeout

const sgMail = require('@sendgrid/mail');

exports.handler = async (event, context) => {
  console.log('🔄 Sync batch checking for inactive chats (3 weeks range)...');
  console.log('📅 Current time:', new Date().toISOString());
  
  const startTime = Date.now();
  const MAX_EXECUTION_TIME = 24000; // 24 seconds (leaving 2s buffer)
  ...
  try {
    ...
    do {
      ...
      pageCount++;
      console.log(`📊 Page ${pageCount}: Fetched ${chatsData.records.length} records, total so far: ${allRecords.length}`);
      
      // Check timeout
      if (Date.now() - startTime > 5000) {
        console.log('⚠️ Data fetching taking too long, proceeding with what we have');
        break;
      }
    } while (offset);
    ...
    // Process synchronously within timeout
    ...
    for (const chat of inactiveChats) {
      ...
      if (Date.now() - startTime > MAX_EXECUTION_TIME) {
        console.log(`⏰ Approaching timeout limit. Processed ${processedCount} of ${inactiveChats.length}`);
        break;
      }
      ...
      try {
        ...
        // Save check-in message - SKIP THIS TO SAVE TIME
        // We'll just send the emails for now
              ...
              <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 24px; overflow: hidden; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04); border: 2px solid #f5f5f5;">
                ...
                <div style="padding: 50px 40px; text-align: center;">
                  ...
                  <p style="font-size: 18px; color: #64748b; margin-bottom: 30px; line-height: 1.6;">
                    It's been a while since your last conversation. ${character.Name} would love to hear from you again!
                  </p>
        ...
        const msg = {
          ...
          subject: `${character.Name} misses you! 💌`,
          html: emailHtml,
          text: `It's been a while since your last chat with ${character.Name}. Continue your conversation anytime!\n\n${chatUrl}`,
          mailSettings: {
            sandboxMode: {
              enable: false
            }
        ...
        emailsSentCount++;
        processedCount++;
        userEmails.push({ email: userEmail, character: character.Name });
        
        // Log progress every 10 emails
        if (emailsSentCount % 10 === 0) {
          console.log(`📊 Progress: ${emailsSentCount} emails sent, ${Date.now() - startTime}ms elapsed`);
        }
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        remaining: inactiveChats.length - processedCount - skippedCount,
        executionTime: `${executionTime}ms`,
        message: `Sent ${emailsSentCount} emails out of ${inactiveChats.length} inactive chats found`,
        errors: errors.length > 0 ? errors.slice(0, 10) : undefined,
        sample: userEmails.slice(0, 5) // Show first 5 emails sent
      })
    };
