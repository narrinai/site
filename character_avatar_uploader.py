...
if __name__ == "__main__":
    ...
    uploader.run(max_characters=5)  # Start met 5 characters voor test, 'growth': 'UP', 'strategy': 'PLAN', 'leader': 'LEAD',
    ...
    def create_text_avatar(self, text, character_name, bg_color='white', text_color='black'):
        """Create a square avatar image with text/symbol"""
        ...
        try:
            # Use a large, reliable font
            font_size = 120  # Start with reasonable size
            try:
                # Try to load system fonts that support text well
                font_paths = [
            ...
            print(f"   🎨 Created text avatar: '{text}' (font size: {font_size}px)")
            
        except Exception as e:
            print(f"   ⚠️ Text rendering error: {e}")
            # Simple fallback
            draw.text((size//2 - 50, size//2 - 20), text, fill=text_color)
